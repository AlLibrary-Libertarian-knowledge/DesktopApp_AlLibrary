...
test.describe('Document community and sharing flows', () => {
  test('add/edit/delete comment and share to peers with toasts', async ({ page }) => {
    ...
    await expect(
      page.getByText(/Comment posted|Comentário publicado|Comentario publicado/)
    ).toBeVisible();
    ...
    await page
      ...
      .click();
    await expect(
      page.getByText(/Comment deleted|Comentário excluído|Comentario eliminado/)
    ).toBeVisible();
    ...
    await expect(
      page.getByText(
        /Shared to selected peers|Compartilhado com os pares selecionados|Compartido con los pares seleccionados/
      )
    ).toBeVisible();
    ...
    await expect(
      page.getByText(
        /Share link copied|Link de compartilhamento copiado|Enlace de compartición copiado/
      )
    ).toBeVisible();
  });
});
