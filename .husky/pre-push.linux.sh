...
# -----------------------------
# AlLibrary Pre-push — Linux / macOS (pnpm, Vite, Vitest, Playwright Chromium)
# -----------------------------
...
on_error() {
  ...
  echo "${YELLOW}Recovery:${RESET} ${BOLD}pnpm run quality:ci${RESET} · ${BOLD}pnpm run husky:analyze:changes${RESET}"
  exit 1
}
...
run_step() {
  ...
  cat "$temp_log" >> .git/prepush.log
  cat "$temp_log" > "$tier_log"
  echo "${RED}✖ ${label}${RESET} ($(( $(date +%s) - ts ))s)"
  cat "$temp_log"
  echo "${YELLOW}Log:${RESET} ${tier_log}"
  rm -f "$temp_log"
  return 1
}
...
# Tier 3
if [ "$RUN_T3_E2E" -gt 0 ] || [ "$RUN_T3_BUILD" -gt 0 ] || [ "$RUN_T3_BUDGET" -gt 0 ] || [ "$RUN_T3_LH" -gt 0 ]; then
  echo "${BLUE}• Tier 3:${RESET} e2e / build"

  if [ "$RUN_T3_E2E" -gt 0 ]; then
    echo "${BLUE}• Step:${RESET} Playwright browsers (chromium)"
    pnpm exec playwright install chromium || exit 1
    if ! run_step "E2E (Chromium)" pnpm exec playwright test --project=chromium; then
      exit 1
    fi
  fi
  ...
  if [ "$RUN_T3_BUDGET" -gt 0 ]; then
    echo "${YELLOW}• Bundle budget not configured (skipped).${RESET}"
  fi
  if [ "$RUN_T3_LH" -gt 0 ]; then
    echo "${YELLOW}• Lighthouse not configured (skipped).${RESET}"
  fi
else
