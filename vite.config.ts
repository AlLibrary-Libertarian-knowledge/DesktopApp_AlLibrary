...
function readAppMeta(mode: string) {
  const env = loadEnv(mode, projectRoot, '');
  const version = env.APP_VERSION || env.VITE_APP_VERSION || '0.0.0-dev';
  const name = env.VITE_APP_NAME || 'AlLibrary';
  return { version, name };
}

// https://vitejs.dev/config/
export default defineConfig(({ mode }) => {
  const { version: appVersion, name: appName } = readAppMeta(mode);
